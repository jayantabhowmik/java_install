...
def install_java():
    ...
    if system == 'Linux':
        ...
        if 'ID' in os_info:
            ...
            if os_id == 'ubuntu':
                ...
                setup_java_home_command = f'export JAVA_HOME={java_home_path}'
                update_bashrc_command = f'echo "export JAVA_HOME={java_home_path}" >> ~/.bashrc'
            elif os_id == 'debian':
    ...
    try:
        ...
        if os_id == 'ubuntu':
            ...
            subprocess.run(setup_java_home_command, shell=True, check=True)
            subprocess.run(update_bashrc_command, shell=True, check=True)
            print("JAVA_HOME set successfully. Please restart your terminal session for changes to take effect.")
