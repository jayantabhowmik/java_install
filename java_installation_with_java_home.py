...
def install_java():
    ...
    if system == 'Linux':
        ...
        if 'ID' in os_info:
            ...
            if os_id == 'ubuntu':
                ...
                setup_java_home_command = f'sudo sh -c "echo \'JAVA_HOME={java_home_path}\' >> /etc/environment"'
                reload_path_command = 'sudo sh -c ". /etc/environment"'
            elif os_id == 'debian':
    ...
    try:
        ...
        if os_id == 'ubuntu':
            ...
            subprocess.run(reload_path_command, shell=True, check=True)
            print("JAVA_HOME set successfully.")
